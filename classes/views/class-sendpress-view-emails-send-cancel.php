...
<h2><?php _e('Cancel Scheduled Email'); ?></h2>
<div class='well'>
    <?php
    $info = get_post_meta($post->ID, '_send_time', true);
    ?>
   <p><?php _e('Subject'); ?>: <?php echo $post->post_title; ?></p>
   <p><?php _e('Date'); ?>: <?php echo date_i18n('Y/m/d @ h:i A' , strtotime( $info ) ); ?></p>
    <?php SendPress_Data::nonce_field(); ?>
    <button class="btn" value="cancel" name="submit"><?php _e('Cancel');?></button>
    <button class="btn btn-danger" value="delete" name="submit"><?php _e('Delete Scheduled Email');?></button>
</div>
</form>
		<?php
	} 
